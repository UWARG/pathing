...
def test_no_laps(non_empty_start_sequence: "list[location_ground.LocationGround]",
            non_empty_lap_sequence: "list[location_ground.LocationGround]"):
    """
    Tests for correct functionality when we pass in no laps
    """
    success, mission = create_mission_from_waypoints.create_mission_from_waypoints(0, TAKEOFF_ALTITUDE, LAPS_ALTITUDE, non_empty_start_sequence, non_empty_lap_sequence)
    assert not success
    assert mission is None


def test_empty_start_sequence(non_empty_lap_sequence: "list[location_ground.LocationGround]"):
    """
    Tests functionaltiy for an empty start sequence
    """
    start_sequence = []
    success, mission = create_mission_from_waypoints.create_mission_from_waypoints(NUM_LAPS, TAKEOFF_ALTITUDE, LAPS_ALTITUDE, start_sequence, non_empty_lap_sequence)
    assert not success
    assert mission is None


def test_empty_lap_sequence(non_empty_start_sequence: "list[location_ground.LocationGround]"):
    """
    Tests functionaltiy for an empty lap sequence
    """
    lap_sequence = []
    success, mission = create_mission_from_waypoints.create_mission_from_waypoints(NUM_LAPS, TAKEOFF_ALTITUDE, LAPS_ALTITUDE, non_empty_start_sequence, lap_sequence)
    assert not success
    assert mission is None


def test_valid_waypoints(non_empty_start_sequence: "list[location_ground.LocationGround]",
                    non_empty_lap_sequence: "list[location_ground.LocationGround]"):
    """
    Ensures that mission is correctly made based on valid parameters
    """
    print("Todo")
    success, mission_actual = create_mission_from_waypoints.create_mission_from_waypoints(NUM_LAPS, TAKEOFF_ALTITUDE, LAPS_ALTITUDE, non_empty_start_sequence, non_empty_lap_sequence)
    assert success
    assert isinstance(mission_actual, list)
    assert len(mission_actual) == (len(non_empty_start_sequence) + NUM_LAPS * len(non_empty_lap_sequence))
