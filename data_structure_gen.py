...
def list(latlon, name):
    ...
    for line in file:
        ...
        if (latlon == True) and (name == True):
            ...
            temp_list.append((name.strip(),utm.from_latlon(float(lat),float(lon))[0],utm.from_latlon(float(lat),float(lon))[1]))
...
def dictionary(latlon, name):
    ...
    for line in file:
        ...
        if latlon == True:
            ...
            temp_dict[name.strip()] = (utm.from_latlon(float(lat),float(lon))[0],utm.from_latlon(float(lat),float(lon))[1])
    return temp_dict